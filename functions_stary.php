...
function sk_log_profile_visit() {
    ...
    if ($profile_user_id == $visitor_user_id) return;
...
if (!function_exists('calculate_match_percentage')) {
    function calculate_match_percentage($user1_id, $user2_id) {
        if ($user1_id == $user2_id) return 100;
...
function load_users_grid_with_preferences_ajax() {
    ...
    if (!is_array($blocked_users_list)) $blocked_users_list = [];
    ...
    $target_gender = '';
    if ($seeking_preference === 'Kobiety') $target_gender = 'Kobieta';
    elseif ($seeking_preference === 'Mężczyzny') $target_gender = 'Mężczyzna';
...
function sk_preferences_stats_shortcode() {
    if (!is_user_logged_in()) return '<p>Musisz być zalogowany, aby zobaczyć statystyki.</p>';
    
    // Ręczne ładowanie pliku wtyczki także tutaj, dla pewności.
    $match_me_plugin_file = WP_PLUGIN_DIR . '/match-me-for-buddypress/match-me-for-buddypress.php';
    if (file_exists($match_me_plugin_file)) require_once($match_me_plugin_file);

    $match_me_obj = null;
    if (class_exists('Mp_BP_Match')) $match_me_obj = new Mp_BP_Match();
    ...
    foreach ($all_users as $user) {
        ...
        if (!empty($preferences['politics']) && $preferences['politics'] !== bp_get_profile_field_data(['field' => 'Polityka', 'user_id' => $user_id])) continue;
        if (!empty($preferences['dieta']) && $preferences['dieta'] !== bp_get_profile_field_data(['field' => 'Dieta', 'user_id' => $user_id])) continue;
        if (!empty($preferences['religia']) && $preferences['religia'] !== bp_get_profile_field_data(['field' => 'Religia', 'user_id' => $user_id])) continue;
        if (!empty($preferences['styl_pracy']) && $preferences['styl_pracy'] !== bp_get_profile_field_data(['field' => 'Styl pracy', 'user_id' => $user_id])) continue;
        
        if ($match_me_obj && is_callable([$match_me_obj, 'hmk_get_matching_percentage_number'])) {
            $match_percentage = $match_me_obj->hmk_get_matching_percentage_number($user_id, $current_user_id);
            if ($match_percentage < ($preferences['min_match'] ?? 0)) continue;
        }
...
function prawidlowa_aktywacja_uzytkownika($user_id, $key, $user) {
    ...
    if (bp_is_active('xprofile')) {
        $mapa_id_pol = [
            1 => 'first_name', 2 => 'last_name', 129 => 'gender', 338 => 'Szukam',
            198 => 'relationship_search', 218 => 'relationship_status', 290 => 'has_children',
            295 => 'want_children', 226 => 'preferencje_jedzeniowe', 136 => 'religion',
            133 => 'approach_to_faith', 157 => 'reincarnation_belief', 160 => 'alternative_spirituality',
            303 => 'zodiac_sign', 108 => 'styl_pracy', 114 => 'luksus', 185 => 'ryzyko',
            298 => 'mieszkanie', 215 => 'polityka_skrot', 190 => 'identyfikacja_polityczna',
            286 => 'alkohol', 236 => 'friendly_420', 316 => 'typ_ciala', 324 => 'cwiczenia',
            329 => 'czyta', 206 => 'jezyki', 'dieta' => 'dieta',
            343 => 'field_343',
            345 => 'field_345',
            107 => 'birth_date'
        ];
    ...
    if ($field_id !== false && !empty($wartosc)) {
        ...
        if (!empty($meta['first_name'])) wp_update_user(['ID' => $user_id, 'first_name' => sanitize_text_field($meta['first_name'])]);
        if (!empty($meta['last_name'])) wp_update_user(['ID' => $user_id, 'last_name' => sanitize_text_field($meta['last_name'])]);
    }
...
function lewy_panel_uzytkownika_shortcode() {
    ...
    <aside class="left-panel-container">
        ...
        <div class="widget">
            <ul class="widget-nav-list">
                ...
                <li><a href="<?php echo bp_loggedin_user_domain() . bp_get_messages_slug(); ?>">✉️ Wiadomości</a>
                    <?php if (function_exists('get_unread_message_count')) { $unread_count = get_unread_message_count(); $style = $unread_count > 0 ? '' : 'style="display: none;"'; echo '<span class="nav-badge" id="badge-wiadomosci" ' . $style . '>' . $unread_count . '</span>'; } ?>
                </li>
            </ul>
        </div>
            ...
            <div class="widget-content accordion-content" style="display: block;">
                <form id="user-filters-form">
                    ...
                    <div style="margin-bottom: 10px;">
                        <label for="filter-poglady" style="font-weight: 500; font-size: 0.9em;  color: #e0e0e0;">W skrócie Polityka:</label>
                        <select name="poglady" id="filter-poglady" class="user-filter-control" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; margin-top: 5px;">
                    ...
                    <div style="margin-bottom: 10px;">
                        <label for="filter-religia" style="font-weight: 500; font-size: 0.9em;  color: #e0e0e0; ">Ogólne Podejście do Wiary:</label>
                        <select name="religia" id="filter-religia" class="user-filter-control" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; margin-top: 5px;">
                            <option value="">Wszystkie</option>
                    ...
                    <div style="margin-bottom: 10px;">
                        <label for="filter-dieta" style="font-weight: 500; font-size: 0.9em;  color: #e0e0e0; ">Dieta:</label>
                        <select name="dieta" id="filter-dieta" class="user-filter-control" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; margin-top: 5px;">
                            <option value="">Wszystkie</option>
                    ...
                    <div style="margin-bottom: 10px;">
                        <label for="filter-styl-pracy" style="font-weight: 500; font-size: 0.9em;  color: #e0e0e0; ">Styl pracy:</label>
                        <select name="styl_pracy" id="filter-styl-pracy" class="user-filter-control" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; margin-top: 5px;">
                            <option value="">Wszystkie</option>
                    ...
                    <div style="margin-bottom: 10px;">
                        ...
                        <select name="numerology" id="filter-numerology" class="user-filter-control" style="width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; margin-top: 5px;">
                            <option value="">Wszystkie</option>
        ...
        <div class="widget">
            ...
            <button id="toggle-numerology-btn" class="widget-button purple" style="width:100%; box-sizing: border-box;">Pokaż Numerologię</button>
        </div>
                        ...
                        <?php foreach ($blocked_user_ids as $blocked_id) :
                            ...
                            <li class="liked-by-item" data-item-id="<?php echo $blocked_id; ?>">
                              <a href="<?php echo bp_members_get_user_url($blocked_id); ?>" title="Zobacz profil">
                                ...
                                <button class="unblock-button" data-user-id="<?php echo $blocked_id; ?>" title="Odblokuj">Odblokuj</button>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                <?php else : ?>
        ...
        <div class="widget">
            <a href="<?php echo bp_loggedin_user_domain() . 'profile/edit/'; ?>" class="widget-button purple">Pytania o dopasowanie</a>
        ...
        <div class="widget">
            ...
            <div class="profile-completion">
                ...
                <div class="completion-details">
                    ...
                    <div class="progress-bar-container"><div class="progress-bar" style="width: <?php echo $percentage; ?>%;"></div></div>
                    <div class="completion-links"><a href="<?php echo bp_loggedin_user_domain() . 'profile/change-avatar/'; ?>">Zmień zdjęcie</a><a href="<?php echo bp_loggedin_user_domain() . 'profile/edit/'; ?>">Uzupełnij profil</a></div>
                </div>
            </div>
        </div>
        ...
        <div class="widget">
            ...
            <ul class="recent-matches-list">
                ...
                if ($match_me_obj) {
                    ...
                    if(!is_array($blocked_user_ids_for_matches)) $blocked_user_ids_for_matches = [];
                    $all_users = get_users(['exclude' => array_merge([$current_user_id], $blocked_user_ids_for_matches)]);
                    $matches = [];
                    foreach ($all_users as $user) {
                        $matches[] = [
                            'user_id' => $user->ID, 'name' => $user->display_name, 'login' => $user->user_login,
                    ...
                    usort($matches, function($a, $b) { return $b['match_percent'] - $a['match_percent']; });
                    ...
                    foreach ($top_matches as $match) { ?>
                        <li class="recent-match-item">
                            ...
                            <a href="<?php echo wp_nonce_url( bp_loggedin_user_domain() . bp_get_messages_slug() . '/compose/?r=' . $match['login'] ); ?>" class="send-message-btn">✉️</a>
                        </li>
                    <?php }
                } else {
    ...
    <style>
    .left-panel-container { display: flex; flex-direction: column; gap: 20px; }
    .widget { background: #fff; border: 1px solid #e0e0e0; border-radius: 8px; padding: 15px; }
    .widget-title { font-size: 1rem; margin: 0 0 10px 0; padding-bottom: 10px; border-bottom: 1px solid #f0f0f0; }
    .accordion-trigger { cursor: pointer; display: flex; justify-content: space-between; align-items: center; }
    .accordion-trigger:hover { color: #0073aa; }
    .widget-arrow { transition: transform 0.3s ease; }
    .widget-arrow.up { transform: rotate(180deg); }
    .accordion-content { display: none; padding-top: 10px; }
    .liked-by-list { list-style: none; padding: 0; margin: 0; max-height: 250px; overflow-y: auto; }
    .liked-by-item { display: flex; align-items: center; justify-content: space-between; padding: 5px 0; gap: 10px; }
    .liked-by-item a { display: flex; align-items: center; gap: 10px; text-decoration: none; color: #333; border-radius: 4px; flex-grow: 1;}
    .liked-by-item a:hover { color: #0073aa; }
    .liked-by-item img { border-radius: 50%; }
    .unblock-button { background: #e0e0e0; border: none; color: #333; font-size: 11px; padding: 4px 8px; border-radius: 3px; cursor: pointer; flex-shrink: 0; }
    .unblock-button:hover { background: #c0c0c0; }
    .no-one-yet { font-size: 0.9rem; color: #666; margin: 5px; }
    .widget-nav-list, .widget-list, .widget-list-toggle, .recent-matches-list { list-style: none; margin: 0; padding: 0; }
    .widget-nav-list li { display: flex; align-items: center; justify-content: space-between; }
    .widget-nav-list a { display: block; padding: 10px 5px; text-decoration: none; color: #333; font-weight: bold; font-size: 1rem; transition: color 0.2s; flex-grow: 1; }
    .nav-badge { background-color: #d9534f; color: white; font-size: 12px; font-weight: bold; padding: 2px 8px; border-radius: 10px; margin-left: 10px; }
    .widget-list li, .widget-list-toggle li { display: flex; justify-content: space-between; align-items: center; font-size: 0.9rem; padding: 5px 0; }
    .widget-list .value { font-weight: bold; color: #333; }
    .widget-list-toggle .value { color: #0073aa; cursor: pointer; }
    .toggle-switch { width: 40px; height: 20px; background: #ccc; border-radius: 10px; position: relative; cursor: pointer; }
    .toggle-switch::before { content: ''; position: absolute; width: 16px; height: 16px; border-radius: 50%; background: white; top: 2px; left: 2px; transition: all 0.2s; }
    .toggle-switch.enabled { background: #4CAF50; }
    .toggle-switch.enabled::before { left: 22px; }
    .widget-button { display: block; text-align: center; padding: 10px; border-radius: 5px; color: white; font-weight: bold; text-decoration: none; margin-bottom: 10px; }
    .widget-button:last-child { margin-bottom: 0; }
    .widget-button.purple { background: #6a1b9a; }
    .widget-button.green { background: #2e7d32; }
    .profile-completion { display: flex; gap: 15px; align-items: center; }
    .completion-avatar { flex-shrink: 0; }
    .completion-avatar img { border-radius: 50%; }
    .completion-details { flex-grow: 1; }
    .completion-details p { margin: 0 0 8px 0; font-size: 0.9rem; }
    .progress-bar-container { width: 100%; background: #e0e0e0; border-radius: 5px; height: 10px; overflow: hidden; }
    .progress-bar { height: 100%; background: #4CAF50; border-radius: 5px; }
    .completion-links { margin-top: 10px; font-size: 0.8rem; }
    .completion-links a { margin-right: 15px; color: #0073aa; text-decoration: none; }
    .recent-match-item { display: flex; align-items: center; gap: 10px; padding: 8px 0; border-bottom: 1px solid #f0f0f0; }
    .recent-match-item:last-child { border-bottom: none; }
    .recent-match-item img { border-radius: 50%; }
    .match-info { display: flex; flex-direction: column; flex-grow: 1; }
    .match-info strong { font-size: 0.9rem; }
    .match-info span { font-size: 0.8rem; color: #666; }
    .send-message-btn { font-size: 1.2rem; text-decoration: none; color: #0073aa; }
    </style>
...
add_action('wp_footer', function() {
	...
	?>
	<style>
		.editable-bp { cursor: pointer; border-bottom: 1px dashed #ccc; }
		.editable-bp:hover { background-color: #f9f9f9; }
		.edit-input-bp { width: 90%; padding: 5px; }
		.edit-controls-bp { margin-top: 8px; }
	</style>
...
function sk_like_button_styles() {
    ...
    <style id="like-button-styles">
        ...
        #buddypress #item-header-content .like-button.liked {
            background-color: #fff0f5; /* Jasnoróżowe tło */
            border-color: #ffb6c1; /* Różowa ramka */
            color: #d6336c; /* Ciemniejszy różowy tekst */
        }

        #buddypress #item-header-content .like-button.liked .heart-icon {
            filter: grayscale(0);
            opacity: 1;
            transform: scale(1.1); /* Lekkie powiększenie serca */
        }
    </style>
    <?php
}
...
function custom_animated_background_css() {
    ...
    <style id="custom-dynamic-background">
        ...
        @keyframes float {
            ...
            50% { transform: translate(-50%, -50%) translateY(-15px) translateX(-20px); }
            75% { transform: translate(-50%, -50%) translateY(-35px) translateX(10px); }
        }
        ...
        @keyframes float-orb-3 {
            0%, 100% { transform: translate(0, 0); }
            20% { transform: translate(60px, -20px); }
...
function rejestracja_krok1_shortcode() {
    ...
    <style>
        .reg-container { max-width: 600px; margin: 40px auto; background: #fff; border-radius: 8px; box-shadow: 0 0 20px rgba(0,0,0,0.05); padding: 40px; }
        .reg-container h1 { font-size: 28px; color: #2e7d32; margin-bottom: 10px; }
        .reg-container label { font-weight: 600; display: block; margin: 20px 0 6px; }
        .reg-container input { width: 100%; padding: 12px; border: 1px solid #ccc; border-radius: 6px; font-size: 16px; }
        .reg-container .note { font-size: 13px; color: #888; margin-top: 4px; }
        .reg-container .radio-group { display: flex; gap: 20px; margin-top: 10px; flex-wrap: wrap; }
        .reg-container .radio-group input { width: auto; }
        .reg-container button { background-color: #2e7d32; color: #fff; border: none; padding: 14px 20px; font-size: 16px; border-radius: 6px; margin-top: 30px; cursor: pointer; width: 100%; }
        .reg-container button:hover { background-color: #1b5e20; }
...
function wyswietl_pasek_postepu($aktualny_krok) {
    ...
    ?>
    <style>
        .pasek-postepu { display: flex; flex-wrap: wrap; list-style: none; padding: 0; margin: 20px 0; gap: 5px; }
        .pasek-postepu li { flex: 1; text-align: center; padding: 10px; background-color: #f0f0f0; border-radius: 8px; min-width: 100px; }
        .pasek-postepu li a { color: inherit; text-decoration: none; display: block; }
        .pasek-postepu li.aktywny { background-color: #a873e8; color: white; font-weight: bold; }
        .pasek-postepu li.ukonczony { background-color: #d8c0f5; color: #333; cursor: pointer; }
        .pasek-postepu li.ukonczony:hover { background-color: #c8aee8; }
        .form-container { padding: 25px; border-radius: 8px; margin-top: 20px; background-color: rgba(255, 255, 255, 0.9); box-shadow: 0 4px 15px rgba(0,0,0,0.1); max-width: 800px; margin-left: auto; margin-right: auto; }
        .form-container h2 { margin-top: 0; color: #2e7d32; }
        .form-container label { display: block; margin: 15px 0 5px; font-weight: 600; }
        .form-container input[type="text"], .form-container input[type="email"], .form-container input[type="password"], .form-container input[type="date"], .form-container select, .form-container textarea { width: 100%; padding: 10px; margin-bottom: 10px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box; }
        .form-container .radio-group, .form-container .checkbox-group { display: flex; flex-direction: column; gap: 8px; margin-bottom: 15px; }
        .form-container .radio-group label, .form-container .checkbox-group label { font-weight: normal; display: flex; align-items: center; }
        .form-container .radio-group input, .form-container .checkbox-group input { width: auto; margin-right: 8px; }
        .form-container input[type="submit"] { background-color: #a873e8; color: white; padding: 12px 25px; border: none; border-radius: 5px; cursor: pointer; font-size: 16px; width: 100%; margin-top: 20px; }
        .form-container input[type="submit"]:hover { background-color: #8e5ccb; }
        .rejestracja-status { padding: 20px; margin: 20px auto; border-radius: 8px; font-size: 18px; text-align: center; font-weight: bold; max-width: 800px; }
        .rejestracja-status.success { background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; }
        .rejestracja-status.error { background-color: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; }
    </style>
    <ul class="pasek-postepu">
        <?php foreach ($kroki as $numer => $nazwa) :
            $klasa = '';
            if ($numer == $aktualny_krok) $klasa = 'aktywny';
            elseif ($numer < $aktualny_krok) $klasa = 'ukonczony';
            ?>
            <li class="<?php echo $klasa; ?>">
                <?php if ($numer < $aktualny_krok && isset($_SESSION['rejestracja'])): ?>
                    <a href="<?php echo esc_url(add_query_arg('krok', $numer, get_permalink())); ?>">Krok <?php echo $numer . ': ' . esc_html($nazwa); ?></a>
                <?php else: ?>
...
$action_url = add_query_arg('krok', $krok + 1, $base_url);
    ...
    <div class="form-container">
        <form action="<?php echo esc_url($action_url); ?>" method="post" <?php if ($krok == 5) echo 'enctype="multipart/form-data"'; ?>>
            <?php
            switch ($krok) {
                case 1: ?>
                    ...
                    <label for="username">*Nazwa użytkownika:</label>
                    <input type="text" name="username" id="username" value="<?php echo esc_attr($dane_sesji['username'] ?? ''); ?>" required maxlength="15">
                    
                    <label for="email">*E-mail:</label>
                    <input type="email" name="email" id="email" value="<?php echo esc_attr($dane_sesji['email'] ?? ''); ?>" required>
                    ...
                    <div class="radio-group">
                        ...
                        <label><input type="radio" name="gender" value="Inna" <?php checked($dane_sesji['gender'] ?? '', 'Inna'); ?> required> Inna</label>
                    </div>
                    
                    <label for="data_urodzenia">*Data urodzenia:</label>
                    <input type="date" name="data_urodzenia" id="data_urodzenia" value="<?php echo esc_attr($dane_sesji['data_urodzenia'] ?? ''); ?>" required>
                    ...
                    <select name="orientacja" id="orientacja">
                        ...
                        <option value="Heteroseksualna" <?php selected($dane_sesji['orientacja'] ?? '', 'Heteroseksualna'); ?>>Heteroseksualna</option>
                        <option value="Homoseksualna" <?php selected($dane_sesji['orientacja'] ?? '', 'Homoseksualna'); ?>>Homoseksualna</option>
                        <option value="Biseksualna" <?php selected($dane_sesji['orientacja'] ?? '', 'Biseksualna'); ?>>Biseksualna</option>
                        <option value="Panseksualna" <?php selected($dane_sesji['orientacja'] ?? '', 'Panseksualna'); ?>>Panseksualna</option>
                        <option value="Aseksualna" <?php selected($dane_sesji['orientacja'] ?? '', 'Aseksualna'); ?>>Aseksualna</option>
                    </select>
                ...
                case 2: ?>
                    ...
                    <div class="radio-group">
                        <label><input type="radio" name="dzieci" value="Tak" <?php checked($dane_sesji['dzieci'] ?? '', 'Tak'); ?>> Tak</label>
                        <label><input type="radio" name="dzieci" value="Nie" <?php checked($dane_sesji['dzieci'] ?? '', 'Nie'); ?>> Nie</label>
                    </div>
                ...
                case 3: ?>
                    ...
                    <div class="radio-group">
                        ...
                        <label><input type="radio" name="religia" value="Własne Przekonania Religijne" <?php checked($dane_sesji['religia'] ?? '', 'Własne Przekonania Religijne'); ?>> Własne Przekonania Religijne</label>
                    </div>
                    ...
                    <div class="radio-group">
                        <label><input type="radio" name="duchowosc_alternatywna" value="Bardzo zainteresowany" <?php checked($dane_sesji['duchowosc_alternatywna'] ?? '', 'Bardzo zainteresowany'); ?>> Bardzo zainteresowany</label>
                        <label><input type="radio" name="duchowosc_alternatywna" value="Średnio zainteresowany" <?php checked($dane_sesji['duchowosc_alternatywna'] ?? '', 'Średnio zainteresowany'); ?>> Średnio zainteresowany</label>
                        <label><input type="radio" name="duchowosc_alternatywna" value="Nie interesuje mnie to" <?php checked($dane_sesji['duchowosc_alternatywna'] ?? '', 'Nie interesuje mnie to'); ?>> Nie interesuje mnie to</label>
                    </div>
                ...
                case 4: ?>
                    ...
                    <label>Dieta:</label>
                    <div class="radio-group">
                        <label><input type="radio" name="dieta" value="Vege" <?php checked($dane_sesji['dieta'] ?? '', 'Vege'); ?>> Vege</label>
                        <label><input type="radio" name="dieta" value="Mięso" <?php checked($dane_sesji['dieta'] ?? '', 'Mięso'); ?>> Mięso</label>
                        <label><input type="radio" name="dieta" value="Inna" <?php checked($dane_sesji['dieta'] ?? '', 'Inna'); ?>> Inna</label>
                    </div>
                    ...
                    <div class="radio-group">
                        ...
                        <label><input type="radio" name="styl_pracy" value="Artysta (praca kreatywna)" <?php checked($dane_sesji['styl_pracy'] ?? '', 'Artysta (praca kreatywna)'); ?>> Artysta (praca kreatywna)</label>
                ...
                case 5: ?>
                    ...
                    <textarea name="about_me" id="about_me" rows="6" minlength="150" required><?php echo esc_textarea($dane_sesji['about_me'] ?? ''); ?></textarea>
...
function wyswietl_formularz_logowania_shortcode() {
    ...
    <style>
        ...
        .custom-login-wrapper label {
            color: #333333 !important; /* Wymuszenie ciemnego koloru etykiet */
            font-weight: 600;
            margin-bottom: 5px;
            display: block;
        }
        ...
        .custom-lost-password a {
            color: #d6336c; /* Kolor pasujący do PrawdziwaMiłość (róż/czerwień) */
            text-decoration: none;
        }
...
$userid = bp_core_signup_user(
    ...
    $usermeta
            ...
            if (is_wp_error($user_id)) {
                $errors[] = "Błąd rejestracji: " . $user_id->get_error_message();
            } else {
                // SUKCES!
...
if (!empty($_POST['dataurodzenia'])) {
    ...
    xprofile_set_field_data($iddataurodzenia, $userid, $datasql);
}
...
function my_force_bp_avatar_html($html, $params) {
    ...
    if (empty($params['item_id'])) return $html;
...
function my_force_bp_avatar_url($url, $params) {
    if (empty($params['item_id'])) return $url;
...
function handle_custom_registration_form() {
    ...
    if ( isset( $_POST['submit_registration'] ) && isset( $_POST['wp_nonce'] ) && wp_verify_nonce( $_POST['wp_nonce'], 'user-registration' ) ) {
        ...
        if ( empty( $username ) || empty( $email ) || empty( $password ) || empty( $gender ) || empty( $birthdate ) ) { return; }
...
function render_step_three_form() {
    ...
    <div class="krok-3-container">
        ...
        <form method="post" class="pm-custom-form">
            ...
            <div class="form-group full-width">
                ...
                <textarea name="bio" rows="5" placeholder="Napisz kilka zdań o sobie, swoich pasjach i marzeniach..."><?php echo xprofile_get_field_data('O mnie', $user_id); ?></textarea>
            </div>
    ...
    <style>
        ...
        .krok-3-container { max_width: 800px; margin: 0 auto; padding: 20px; }
        .pm-custom-form { background: #fff; padding: 30px; border-radius: 15px; box-shadow: 0 4px 20px rgba(0,0,0,0.05); }
        ...
        .pm-btn-primary { background: #E1306C; color: #fff; padding: 15px 30px; border: none; border-radius: 50px; cursor: pointer; font-size: 18px; width: 100%; transition: 0.3s; }
        .pm-btn-primary:hover { background: #C13584; }
        .pm-success-msg { background: #d4edda; color: #155724; padding: 15px; border-radius: 8px; margin-bottom: 20px; text-align: center; }
...
function inteligentna_naprawa_kolorow_js() {
    ...
    #nav a, 
    #backtoblog a {
        color: #ffffff !important; /* Białe linki */
...
function pm_strict_match_validation( $errors, $recipients ) {
    ...
    // WAŻNE: Odkomentuj poniższe linie tylko jeśli Admin TEŻ ma mieć blokadę.
    // Domyślnie Admin może pisać do wszystkich (dobre do supportu).
    /*
    ...
    */
    ...
    foreach ( $recipients as $recipient ) {
        ...
        if ( function_exists( 'friends_check_friendship' ) ) {
            $is_match = friends_check_friendship( $sender_id, $recipient_id );
        }

        // Jeśli NIE MA matcha, dodajemy błąd krytyczny do obiektu $errors
        if ( ! $is_match ) {
            // Dodajemy błąd. W API zwróci to kod 400/500 z tą wiadomością.
            $errors->add( 
                'no_match_error', 
...
function pm_hide_message_button_no_match() {
    ...
    $is_match = false;
    if ( function_exists( 'friends_check_friendship' ) ) {
        $is_match = friends_check_friendship( $user_id, $displayed_user_id );
    }

    // Jeśli admin, zawsze pokazuj (opcjonalne)
    if ( current_user_can('manage_options') ) {
        $is_match = true;
    }

    // Jeśli nie ma matcha, usuwamy przycisk
    if ( ! $is_match ) {
        bp_core_remove_nav_item( 'messages' ); // Usuwa zakładkę
        
        // To usuwa główny przycisk na headerze profilu
        remove_action( 'bp_member_header_actions',    'bp_send_private_message_button', 20 );
        
        // Dodatkowy filtr dla pewności (niektóre motywy używają tego)
...
function zmiana_nazwy_zakladki_friends() {
    buddypress()->members->nav->edit_nav( array(
        'name' => 'Dopasowania', // Tu wpisz nową nazwę
        'slug' => 'dopasowania'  // Opcjonalnie zmiana sluga w URL
    ), 'friends' );
}
